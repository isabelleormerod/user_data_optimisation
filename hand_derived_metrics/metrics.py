...
Usage:
  python metrics.py --landmarks-root A:\\Automated_chain_BETA\\Participant_Landmarks [--participants P007 P008] [--out combined.csv]
...
def metrics_for_pen(pen_path: Path) -> pd.DataFrame:
    ...
    if not df.empty:
        df["trial_num"] = df.groupby("height").cumcount() + 1
    return df
